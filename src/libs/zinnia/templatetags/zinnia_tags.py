...
@register.inclusion_tag('zinnia/tags/dummy.html', takes_context=True)
def get_plain_categories(context,template='zinnia/tags/clear.html'):
    cats=[]
    if Category.published.all():
        for i in Category.published.all():
            cats.append(i.title)
...
@register.simple_tag
def get_object_categories(entry):
    cats= ' '
    if entry:
        cat=entry.categories.all()
        for i in cat:
           cats+=' '+i.title
